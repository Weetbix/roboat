...
Roboat.prototype.run = function(){
	...
	// Post daily facts about what happened on this date
	var self = this;
	schedule.scheduleJob("0 0 10 * * 1-5", function(){
		var now = new Date();
		history.getRandomHistoryItemForDate(now.getDate(), now.getMonth() + 1)
		.then(function(eventJson){
			var goodSynonmys = ["Convenient", "Healthy", "Proficient", "Kind", "Good", "Irreproachable", "Principled", "Beneficial"];
			var msg = _.sample(goodSynonmys) + " morning humans.\n" +
					  "On this day in the year " + eventJson.year + ", " + eventJson.text;
							
			self.postMessageToChannel("general", msg, { as_user: true });
		})
		.fail(function(err){
			console.log("Error when trying to post daily history: " + err);	
		});
	});
	// Christmas message
	schedule.scheduleJob("0 0 12 25 12 *", function(){
		self.postMessageToChannel("general", "Healthy Christmas, humans.", { as_user: true });
	});
};
...
Roboat.prototype._isBotMessage = function(message){
