...
Roboat.prototype.run = function(){
	...
	this.on('message', this._responseToTweetRequests);
};
...
Roboat.prototype._responseToTweetRequests = function(message){
	if(this._isChatMessage(message) &&
	   !this._isMessageFromRoboat(message) &&
	   ...
	   S(message.text.toLowerCase()).contains('roboat'))
	{
		...
		this.twitter_client.get('search/tweets',
			{q: tweet_query, count: 1, result_type: "popular"},
			function(err, tweets, response) {
				var reply;
				
				if (err)
				{
					reply = "*BZZtZapZang1010*. Something went wrong.";
				}
				else if (tweets)
				{
					var tweet = tweets.statuses[0];
					reply = "Calculating *" + tweet_query + "*.\n";
					reply += ">>>" + tweet.text + "\n";
